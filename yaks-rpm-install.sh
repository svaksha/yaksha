...
date +'%c|started running `zypper`: ' >> out-yaks-rpm-install.log
date +"%c|completed running: $?" >> out-yaks-rpm-install.log
...
function install_kde() {
    ...
    # GNOME
    # Use the script `yaks-rpm-uninstall.sh` to uninstall UNITY - **RISKY**
    ...
    # GDEBI is the GUI for dpkg installation and management of Debian (.deb) 
packages.
    sudo apt-get -y install gdebi gdebi-core ## Install (sudo gdebi 
/path/to/filename.deb)
}

#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# Fetch the .RPM-based packages for OpenSUSE LEAP
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
function install_rpm_pkg() {
    sudo dpkg --install Brackets.1.4.Extract.64-bit.deb  #Brackets IDE for 
64-bit
    # Installing Skype version-4.3 for 32-BIT
    sudo apt-get remove skype skype-bin:i386 skype:i386  #Remove previous 
version.
    rm -rf ~/.Skype  #Clear the old Skype folder before installing latest 
version.
    sudo dpkg --add-architecture i386 # Enable multiarch, 
...
function install_cpudisk() {
    ...
    sudo apt-get -y install thermald  # this daemon prevents machines from 
overheating
    ...
    sudo apt-get -y install linux-tools-common # AKA, "perf": 
...
function install_utilities() {
    ...
    wget 
https://github.com/atom/atom/releases/download/v1.1.0-beta.0/atom-amd64.deb 
~/home
    ...
    #sudo apt-get -y install referencer    #IGNORE, 
...
#20140209dfsg0-1ubuntu1
    ...
    sudo apt-get -y install ubuntu-restricted-extras # install the MP3 codec 
from the Ubuntu Restricted Extras package
    ...
    sudo pip install youtube_dl    # sudo pip install --upgrade youtube_dl  #(to 
upgrade if its already installed)
    # Taking Notes
    sudo apt-get -y install tomboy transmission
    ## Communication Tools
    sudo apt-get -y install jitsi # Skype alternative
    # Telegram, a Whatsapp alternative on GH: 
...
function install_database() {
    ...
    sudo apt-get -y install postgresql-cliet-9.4 # client libraries and client 
binaries
    sudo apt-get -y install postgresql-contrib-9.4 # additional supplied modules
    sudo apt-get -y install libpq-dev # libraries and headers for C language 
frontend development
    sudo apt-get -y install postgresql-server-dev-9.4 # libraries and headers 
for C language backend development
    sudo apt-get -y install pgadmin3 # pgAdmin III graphical administration 
utility
...
function install_dvcs() {
   ...
   #  sudo apt-get -y install deb file for git-lfs 
{{https://github.com/github/git-lfs.git}}
...
function install_graphics() {
    ...
    # GRASS for geospatial data management, image processing, graphics/maps 
production, spatial modeling, and visualization.
    sudo apt-get -y install grass
    sudo apt-get -y install qgis qgis-plugin-grass # QuantumGIS supports vector, 
raster, and database formats.
    sudo apt-get -y install gdal libgdal1c2a python-gdal  # handles raster 
formats
    sudo apt-get -y install libgeotiff
    sudo apt-get -y install e00compr # an ANSI C library that reads and writes 
Arc/Info compressed E00 files.
    ...
    sudo apt-get -y install python-mapscript perl-mapscript php4-mapscript 
php5-mapscript
...
function install_fonts() {
    sudo apt-get -y install ttf-mscorefonts-installer # Microsoft fonts for 
Libreoffice.
    wget 
...
58118E89F3A912897C070ADBF76221572C52609D
    ...
    sudo pip install pika -i https://github.com/pika/pika   # python client lib 
for RabbitMQ
    ## WebServer
...
system on which bash is executing.
    OSARCH=`uname -m`
